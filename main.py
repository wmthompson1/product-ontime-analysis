...
def proxy_define_relationship(path):
    """Proxy /define-relationship/* → HF Space static build on port 8080."""
    ...
    try:
        ...
        return Response("The HF Space is starting up. Please refresh in a moment.", status=503)

@app.route('/mcp/', defaults={'path': ''})
@app.route('/mcp/<path:path>', methods=['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'OPTIONS'])
def proxy_mcp(path):
    """Proxy /mcp/* → HF Space MCP tool endpoints on port 8080."""
