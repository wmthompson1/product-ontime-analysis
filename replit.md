...
- **Production Dispatcher (Hybrid RAG)**: Closed-vocabulary semantic router that maps natural language questions to Intent/Concepts/Perspective using OpenAI GPT-4o-mini (with mock keyword fallback). Routes through SolderEngine for governed SQL assembly. LLM acts as classifier only—never generates SQL. Supports OUT_OF_SCOPE detection, perspective override, and multi-dialect output. Integrated as "Ask a Question" Gradio tab.
