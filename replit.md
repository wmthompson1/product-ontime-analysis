...
| File | What it covers |
|---|---|
| `tests/test_field_description_pipeline.py` | Drafts (plain-language, no SQL jargon), KB-context selection, CSV round-trip, graph coverage (230/230), overlay-only guardrail (15 tests) |
...
- `replit_integrations/field_description_coverage_check.py` — proves `field_descriptions.csv` covers every graph column node in `graph_metadata.json` exactly (230/230), every description non-empty, and no extra rows. File-vs-file (no DB/network needed).
