...
DECLARE @LOT_ID NVARCHAR(3) = N'1';
DECLARE @SPLIT_ID NVARCHAR(3) = N'3';
DECLARE @SUB_ID NVARCHAR(3) = NULL;
...
declare @specs_Different BIT = null;
  ...
  from dbo.WORK_ORDER a
   -- on i.WORKORDER_BASE_ID=a.BASE_ID
    ...
    LEFT JOIN dbo.OPERATION_TYPE ot
...
-- on o.OPERATION_TYPE = ko.OPERATION_TYPE_ID
    ...
    AND (a.BASE_ID in ( @WorkorderBaseId)
		    OR @TEST1 = 1)
    AND (a.LOT_ID = @LOT_ID OR @LOT_ID IS NULL)
    AND (a.SPLIT_ID = @SPLIT_ID OR @SPLIT_ID IS NULL)
