...
/****** Object:  StoredProcedure [dbo].[SP_SK_ETL01_WS]    Script Date: 1/19/2026 12:42:35 PM ******/
SET ANSI_NULLS ON
GO
...
-- Update: JC 20250325 - updated linked server SQL-BI-1 to SQL-LAB-1
*/

										  

--CREATE PROCEDURE [dbo].[SP_SK_ETL01_WS] AS
...
SET @cols = STUFF( (  SELECT DISTINCT ',' + QUOTENAME(c.Report_Section) 
	FROM LIVESupplemental.dbo.ReportSettings_ResourceID c
...
SELECT [TYPE] = SUBSTRING(ud.DOCUMENT_ID, 0, CHARINDEX(N'~',ud.DOCUMENT_ID))
		...
		, SPLIT_ID = SUBSTRING(ud.DOCUMENT_ID, CHARINDEX(N'~', ud.DOCUMENT_ID, CHARINDEX(N'~', ud.DOCUMENT_ID, CHARINDEX(N'~',ud.DOCUMENT_ID, CHARINDEX(N'~',ud.DOCUMENT_ID) + 1) + 1) + 1, 1) 
...
SELECT wo.BASE_ID, wo.LOT_ID, wo.SPLIT_ID, wo.SUB_ID, wo.[TYPE]
	, CASE 
		WHEN wo.PART_ID = N'REWORK MFG' THEN 
			(	SELECT PART_ID FROM WORK_ORDER w 
				WHERE w.[TYPE] = wo.[TYPE] AND w.BASE_ID = wo.BASE_ID 
				AND w.LOT_ID = wo.LOT_ID AND w.SPLIT_ID = wo.SPLIT_ID 
	...
	, CASE   -- added 8/22/2019 VME
		WHEN WO.DESIRED_RLS_DATE IS NULL 
			THEN (	SELECT W.DESIRED_RLS_DATE FROM WORK_ORDER W
					WHERE W.[TYPE] = WO.[TYPE] AND W.BASE_ID = WO.BASE_ID AND W.LOT_ID = WO.LOT_ID 
					AND W.SPLIT_ID = WO.SPLIT_ID AND W.SUB_ID = N'0' )
	...
	, p.COMMODITY_CODE, p.[DESCRIPTION]
	...
	, (	SELECT TOP 1 STRING_VAL FROM USER_DEF_FIELDS ud
		WHERE ud.PROGRAM_ID = N'VMMFGWIN_WO' 
			AND ud.ID = N'UDF-0000070' AND ISNULL(ud.LABEL, '') = ''
			AND wo.[TYPE] + N'~' + wo.BASE_ID + N'~' + N'0' + N'~' + wo.LOT_ID + N'~' + wo.SPLIT_ID = ud.DOCUMENT_ID
	) AS [EXP/FLT]
	, col.user_10, col.USER_1 AS 'col_user_1'
	, CASE 
		WHEN wo.PART_ID = N'REWORK MFG' 
			THEN (	SELECT PART_ID FROM WORK_ORDER w 
					WHERE w.[TYPE] = wo.[TYPE] AND w.BASE_ID = wo.BASE_ID 
						AND w.LOT_ID = wo.LOT_ID AND w.SPLIT_ID = wo.SPLIT_ID 
						AND w.SUB_ID = '0' AND w.PART_ID <> N'REWORK MFG'
			)
		ELSE wo.PART_ID 
	END AS 'RWPart'
	, co.CUSTOMER_PO_REF
	...
	, COL.UNIT_PRICE, COL.TOTAL_AMT_SHIPPED
	, COL.TOTAL_AMT_ORDERED AS 'COL_AMOUNT'
INTO #cte_wo
FROM WORK_ORDER wo2
JOIN WORK_ORDER wo ON wo2.[TYPE] = wo.[TYPE] AND wo2.[BASE_ID] = wo.[BASE_ID]
	AND wo2.[LOT_ID] = wo.[LOT_ID] AND wo2.[SPLIT_ID] = wo.[SPLIT_ID] AND wo2.[SUB_ID] = wo.[SUB_ID]
LEFT JOIN WORK_ORDER EndItem ON EndItem.[TYPE] = wo.[TYPE] AND EndItem.[BASE_ID] = wo.[BASE_ID]
	AND EndItem.[LOT_ID] = wo.[LOT_ID] AND EndItem.[SPLIT_ID] = wo.[SPLIT_ID] AND EndItem.[SUB_ID] = '0'
LEFT JOIN WORKORDER_BINARY wb ON wb.WORKORDER_BASE_ID = wo.BASE_ID AND wb.WORKORDER_LOT_ID = wo.LOT_ID
	AND wb.WORKORDER_SPLIT_ID = wo.SPLIT_ID AND wb.WORKORDER_SUB_ID = wo.SUB_ID AND wb.WORKORDER_TYPE = wo.[TYPE] 
LEFT JOIN DEMAND_SUPPLY_LINK dsl ON dsl.SUPPLY_BASE_ID = wo.BASE_ID AND dsl.SUPPLY_LOT_ID = wo.LOT_ID
	AND dsl.SUPPLY_SPLIT_ID = wo.SPLIT_ID
LEFT JOIN CUST_ORDER_LINE col ON col.CUST_ORDER_ID = dsl.DEMAND_BASE_ID AND col.LINE_NO = dsl.DEMAND_SEQ_NO
	AND dsl.DEMAND_TYPE = N'CO'
LEFT JOIN CUSTOMER_ORDER co ON co.ID = col.CUST_ORDER_ID
LEFT JOIN #NOTATIONBYOWNER n ON n.OWNER_ID = co.ID
JOIN PART_SITE_VIEW p ON p.PART_ID = wo.PART_ID AND p.SITE_ID = N'SK01'
left JOIN PART_SITE_VIEW EndItemPart ON EndItemPart.PART_ID = EndItem.PART_ID
	AND EndItemPart.SITE_ID = N'SK01' AND EndItemPart.PART_ID <> 'REWORK MFG'
	outer APPLY (	SELECT top 1 pud2.DOCUMENT_ID, pud2.id, pud2.LABEL
						, NULLIF(pud2.STRING_VAL,'') AS Material, pud2.PROGRAM_ID, p.id part_id
					from USER_DEF_FIELDS pUD2
					JOIN PART p on pud2.DOCUMENT_ID = p.id
					where pUD2.ID in ('UDF-0000035') -- material
					AND pUD2.PROGRAM_ID = 'VMPRTMNT' AND pUD2.DOCUMENT_ID IS NOT NULL
					AND wo.PART_ID = pUD2.DOCUMENT_ID
	) Material
	outer APPLY (	SELECT top 1 pud3.document_id, pud3.id, pud3.label
						, nullif(pud3.string_val,'') AS alloy, pud3.program_id, p3.id part_id
					FROM user_def_fields pud3
					JOIN PART p3 on pud3.document_id = p3.id
...
LEFT JOIN LIVESupplemental.dbo.SK_CUST_SPECIFIC_PARTS cp ON cp.PART_ID = p.PART_ID
WHERE wo.[STATUS] IN (N'R', N'F', N'U') AND wo.[TYPE] = N'W'
...
; WITH cte_LastClockOut AS (
	SELECT A.WORKORDER_BASE_ID, A.WORKORDER_LOT_ID, A.WORKORDER_SPLIT_ID , A.WORKORDER_SUB_ID
		, A.WORKORDER_TYPE, A.LAST_CLOCK_OUT, MAX(OPERATION_SEQ_NO) AS LAST_OPERATION
	FROM (	SELECT LT.WORKORDER_BASE_ID, LT.WORKORDER_LOT_ID, LT.WORKORDER_SPLIT_ID
				, LT.WORKORDER_SUB_ID, LT.WORKORDER_TYPE, MAX(LT.CLOCK_OUT) AS LAST_CLOCK_OUT
			FROM LABOR_TICKET LT
			WHERE EXISTS (	SELECT 1 FROM #cte_wo WO
					WHERE LT.WORKORDER_BASE_ID = WO.BASE_ID AND LT.WORKORDER_LOT_ID = WO.LOT_ID
						AND LT.WORKORDER_SPLIT_ID = WO.SPLIT_ID AND LT.WORKORDER_SUB_ID = WO.SUB_ID
						AND LT.WORKORDER_TYPE = WO.[TYPE]
			)
			GROUP BY LT.WORKORDER_BASE_ID, LT.WORKORDER_LOT_ID, LT.WORKORDER_SPLIT_ID
				, LT.WORKORDER_SUB_ID, LT.WORKORDER_TYPE
		) A
	JOIN LABOR_TICKET LT ON A.WORKORDER_BASE_ID = LT.WORKORDER_BASE_ID AND A.WORKORDER_LOT_ID = LT.WORKORDER_LOT_ID
		AND A.WORKORDER_SPLIT_ID = LT.WORKORDER_SPLIT_ID AND A.WORKORDER_SUB_ID = LT.WORKORDER_SUB_ID
		AND A.WORKORDER_TYPE = LT.WORKORDER_TYPE AND A.LAST_CLOCK_OUT = LT.CLOCK_OUT
	GROUP BY A.WORKORDER_BASE_ID, A.WORKORDER_LOT_ID, A.WORKORDER_SPLIT_ID
		, A.WORKORDER_SUB_ID, A.WORKORDER_TYPE, A.LAST_CLOCK_OUT
)

, cte_labortickets AS (	SELECT x.* 
	FROM ( SELECT lt.* , a.last_clock_out, a.last_operation
		, rn = ROW_NUMBER() OVER ( PARTITION BY lt.workorder_type , lt.workorder_base_id, lt.workorder_lot_id
			, lt.workorder_split_id, lt.workorder_sub_id, lt.operation_seq_no, lt.clock_out
			ORDER BY lt.transaction_id DESC )
		FROM labor_ticket lt (nolock)
		JOIN cte_lastclockout a ON a.workorder_base_id = lt.workorder_base_id AND a.workorder_lot_id = lt.workorder_lot_id
			AND a.workorder_split_id = lt.workorder_split_id AND a.workorder_sub_id = lt.workorder_sub_id
			AND a.workorder_type = lt.workorder_type AND a.last_operation = lt.operation_seq_no
			AND a.last_clock_out = lt.clock_out
	) X
	WHERE (1=1) AND x.rn = 1
)

--> TT20210727: add last closed date
, CTE_LASTOPCLOSED AS ( SELECT X.WORKORDER_BASE_ID, X.WORKORDER_LOT_ID, X.WORKORDER_SPLIT_ID, X.WORKORDER_SUB_ID
		, X.WORKORDER_TYPE
		, CASE
	...
	FROM (	SELECT O.WORKORDER_BASE_ID, O.WORKORDER_LOT_ID, O.WORKORDER_SPLIT_ID
				, O.WORKORDER_SUB_ID, O.WORKORDER_TYPE, MAX(O.CLOSE_DATE) AS LAST_CLOSED
			FROM OPERATION O
			WHERE EXISTS ( SELECT 1 FROM #cte_wo WO WHERE O.WORKORDER_BASE_ID = WO.BASE_ID
				AND O.WORKORDER_LOT_ID = WO.LOT_ID AND O.WORKORDER_SPLIT_ID = WO.SPLIT_ID
				AND O.WORKORDER_SUB_ID = WO.SUB_ID AND O.WORKORDER_TYPE = WO.[TYPE]
			)
			GROUP BY O.WORKORDER_BASE_ID, O.WORKORDER_LOT_ID, O.WORKORDER_SPLIT_ID
				, O.WORKORDER_SUB_ID, O.WORKORDER_TYPE
	) X
	JOIN OPERATION O ON X.WORKORDER_BASE_ID = O.WORKORDER_BASE_ID AND X.WORKORDER_LOT_ID = O.WORKORDER_LOT_ID
		AND X.WORKORDER_SPLIT_ID = O.WORKORDER_SPLIT_ID AND X.WORKORDER_SUB_ID = O.WORKORDER_SUB_ID
		AND X.WORKORDER_TYPE = O.WORKORDER_TYPE AND X.LAST_CLOSED = O.CLOSE_DATE
	JOIN WORK_ORDER WO ON X.WORKORDER_BASE_ID = WO.BASE_ID AND X.WORKORDER_LOT_ID = WO.LOT_ID
		AND X.WORKORDER_SPLIT_ID = WO.SPLIT_ID AND X.WORKORDER_SUB_ID = WO.SUB_ID AND X.WORKORDER_TYPE = WO.[TYPE]
	GROUP BY X.WORKORDER_BASE_ID, X.WORKORDER_LOT_ID, X.WORKORDER_SPLIT_ID, X.WORKORDER_SUB_ID
		, X.WORKORDER_TYPE, WO.[STATUS], X.LAST_CLOSED
)
--SELECT * FROM CTE_LASTOPCLOSED --WHERE WORKORDER_BASE_ID = '1657144'

, cte_currentoperation AS ( 
SELECT w.*, ISNULL(o.RESOURCE_ID, o2.RESOURCE_ID) RESOURCE_ID_Raw
	, o.OPERATION_TYPE, o2.OPERATION_TYPE AS 'StatusCurrOpType', w.CurrentOperation AS 'CURROPCHK'
	, CASE 
	...
	FROM #cte_wo w 
	LEFT JOIN OPERATION o ON o.WORKORDER_BASE_ID = w.BASE_ID --LAST OP COMPLETE BASED ON STATUS C OR LTICKETS
		AND o.WORKORDER_LOT_ID = w.LOT_ID AND o.WORKORDER_SPLIT_ID = w.SPLIT_ID
		AND o.WORKORDER_SUB_ID = w.SUB_ID AND o.WORKORDER_TYPE = w.[TYPE] 
		AND o.SEQUENCE_NO = w.CurrentOperation
	LEFT JOIN OPERATION o2 ON o2.WORKORDER_BASE_ID = w.BASE_ID -- BASED ON LAST OP WITH STATUS C OR IF NONE, FIRST OP
		AND o2.WORKORDER_LOT_ID = w.LOT_ID AND o2.WORKORDER_SPLIT_ID = w.SPLIT_ID
		AND o2.WORKORDER_SUB_ID = w.SUB_ID AND o2.WORKORDER_TYPE = w.[TYPE]
		AND o2.SEQUENCE_NO = w.StatusCurrOp
	LEFT JOIN (SELECT o.WORKORDER_BASE_ID, o.WORKORDER_LOT_ID, o.WORKORDER_SPLIT_ID, o.WORKORDER_SUB_ID
					, o.WORKORDER_TYPE, o.SEQUENCE_NO, o.RESOURCE_ID
				FROM OPERATION O
				JOIN (	SELECT o.WORKORDER_BASE_ID, o.WORKORDER_LOT_ID, o.WORKORDER_SPLIT_ID, o.WORKORDER_SUB_ID
							, o.WORKORDER_TYPE, MIN(SEQUENCE_NO) minAubPriSequence
						FROM OPERATION o
						WHERE EXISTS(SELECT 1 FROM #cte_wo w
										WHERE w.BASE_ID = o.WORKORDER_BASE_ID AND w.LOT_ID = o.WORKORDER_LOT_ID
											AND w.SPLIT_ID = o.WORKORDER_SPLIT_ID AND w.SUB_ID = o.WORKORDER_SUB_ID
											AND w.[TYPE] = o.WORKORDER_TYPE AND o.SEQUENCE_NO >= w.CurrentOperation
							)
						AND EXISTS(SELECT 1 FROM @ReportSettings s WHERE o.RESOURCE_ID = s.[Resource]
								AND ISNULL(s.HasAubTc, s.HasAubPri) IS NOT NULL
						)
						GROUP BY o.WORKORDER_BASE_ID, o.WORKORDER_LOT_ID, o.WORKORDER_SPLIT_ID
							, o.WORKORDER_SUB_ID, o.WORKORDER_TYPE
				) a ON a.WORKORDER_BASE_ID = o.WORKORDER_BASE_ID AND a.WORKORDER_LOT_ID = o.WORKORDER_LOT_ID
					AND a.WORKORDER_SUB_ID = o.WORKORDER_SUB_ID AND a.WORKORDER_SPLIT_ID = o.WORKORDER_SPLIT_ID
					AND a.WORKORDER_TYPE = o.WORKORDER_TYPE AND a.minAubPriSequence = o.SEQUENCE_NO
	) op ON op.WORKORDER_BASE_ID = w.BASE_ID AND op.WORKORDER_LOT_ID = w.LOT_ID
		AND op.WORKORDER_SPLIT_ID = w.SPLIT_ID AND op.WORKORDER_SUB_ID = w.SUB_ID AND op.WORKORDER_TYPE = w.[TYPE]
	LEFT JOIN OPERATION_BINARY ob ON ob.WORKORDER_BASE_ID = op.WORKORDER_BASE_ID
		AND ob.WORKORDER_LOT_ID = op.WORKORDER_LOT_ID AND ob.WORKORDER_SPLIT_ID = op.WORKORDER_SPLIT_ID
		AND ob.WORKORDER_SUB_ID = op.WORKORDER_SUB_ID AND ob.WORKORDER_TYPE = op.WORKORDER_TYPE
		AND ob.SEQUENCE_NO = op.SEQUENCE_NO
		AND EXISTS(SELECT 1 FROM @ReportSettings s WHERE s.[Resource] = op.RESOURCE_ID
			AND ISNULL(s.HasAubPri, -1) <> -1
		)
	LEFT JOIN OPERATION_BINARY otb ON otb.WORKORDER_BASE_ID = op.WORKORDER_BASE_ID
		AND otb.WORKORDER_LOT_ID = op.WORKORDER_LOT_ID AND otb.WORKORDER_SPLIT_ID = op.WORKORDER_SPLIT_ID
		AND otb.WORKORDER_SUB_ID = op.WORKORDER_SUB_ID AND otb.WORKORDER_TYPE = op.WORKORDER_TYPE
		AND otb.SEQUENCE_NO = op.SEQUENCE_NO
		AND EXISTS(SELECT 1 FROM @ReportSettings s WHERE op.RESOURCE_ID = s.[Resource]
				AND ISNULL(s.HasAubTc, -1) <> -1
			)
)
--SELECT * FROM cte_currentoperation

SELECT w.BASE_ID, w.LOT_ID, w.SPLIT_ID, w.SUB_ID, w.[TYPE], w.PART_ID, w.WONUM, w.DESIRED_QTY
	, w.[STATUS], w.CUSTOMER_ORDER, w.LINE_NO, CUSTOMER_ID = COALESCE(CUSTOMER_ID2, CUSTOMER_ID)
	, w.allocation_type, w.CurrentOperation, w.StatusCurrOp, w.PRINTED_DATE, w.DESIRED_RLS_DATE
	, w.CREATE_DATE, w.remaining_operations, w.QTY_ON_HAND, w.QTY_IN_DEMAND, w.BUYER_USER_ID
	, w.PLANNER_USER_ID, w.COMMODITY_CODE, w.[DESCRIPTION], w.DESIRED_WANT_DATE, w.[EXP/FLT]
	, w.user_10, w.col_user_1, w.RWPart, w.CUSTOMER_PO_REF, w.WORK_ORDER_NOTES, w.CUST_ORDER_NOTATION
	...
	, w.Alloy, w.CO_AMOUNT, w.UNIT_PRICE, w.TOTAL_AMT_SHIPPED, w.COL_AMOUNT, w.RESOURCE_ID_Raw
	, w.OPERATION_TYPE, w.StatusCurrOpType, w.CURROPCHK, w.RESOURCE_ID, w.AubPriText, w.AubTcText
	, w.HasAubPri, w.HasAubTc, w.TimeLimit, l.LAST_CLOCK_OUT , l.LAST_OPERATION AS last_worked_seq
	--> TT20210727: add last closed date
	, LO.LAST_CLOSED
	...
	--> TT20210727: add last closed date
--	, ISNULL(DATEDIFF(dd, w.DESIRED_WANT_DATE, GETDATE()), 0) WAIT_DAYS
	...
	LEFT JOIN PART_SITE_VIEW P ON P.PART_ID = w.PART_ID
		AND P.SITE_ID = 'SK01' AND w.SUB_ID <> N'0'
	LEFT JOIN LIVESupplemental.dbo.buyer_assn leg ON leg.Account_ID = LEFT(P.BUYER_USER_ID, 2)
	LEFT JOIN (	SELECT PO.ID, dslPO.DEMAND_BASE_ID, DEMAND_LOT_ID, DEMAND_SUB_ID, DEMAND_SPLIT_ID
					, dslPO.DEMAND_SEQ_NO, PO.DESIRED_RECV_DATE, PO.PROMISE_SHIP_DATE 
				FROM PURCHASE_ORDER PO
				JOIN DEMAND_SUPPLY_LINK dslPO ON dslPO.SUPPLY_BASE_ID = PO.ID
				GROUP BY PO.ID, dslPO.DEMAND_BASE_ID, DEMAND_LOT_ID, DEMAND_SUB_ID
					, DEMAND_SPLIT_ID, dslPO.DEMAND_SEQ_NO, PO.DESIRED_RECV_DATE, PO.PROMISE_SHIP_DATE
	) dslPO ON dslPO.DEMAND_BASE_ID = w.BASE_ID AND dslPO.DEMAND_LOT_ID = w.LOT_ID
		AND dslPO.DEMAND_SUB_ID = w.SUB_ID AND dslPO.DEMAND_SPLIT_ID = w.SPLIT_ID
		AND dslPO.DEMAND_SEQ_NO = w.CurrentOperation
	LEFT JOIN @ReportSettings rs ON w.RESOURCE_ID_Raw = rs.Resource

	--SELECT * FROM #temp1 where base_id = '1659369'
	--SELECT * FROM #temp1 where PART_ID = '415Z4003-5'


TRUNCATE TABLE Staging.WODS_Output01_ETL
INSERT INTO Staging.WODS_Output01_ETL ( [BASE_ID], [LOT_ID], [SPLIT_ID], [SUB_ID], [TYPE], [PART_ID]
	, [WONUM], [DESIRED_QTY], [STATUS], [CUSTOMER_ORDER], [LINE_NO], [CUSTOMER_ID], [allocation_type]
	, [CurrentOperation], [StatusCurrOp], [PRINTED_DATE], [DESIRED_RLS_DATE], [CREATE_DATE]
	, [remaining_operations], [QTY_ON_HAND], [QTY_IN_DEMAND], [BUYER_USER_ID], [PLANNER_USER_ID]
	, [COMMODITY_CODE], [DESCRIPTION], [DESIRED_WANT_DATE], [EXP/FLT], [user_10], [col_user_1]
	, [RWPart], [CUSTOMER_PO_REF], [WORK_ORDER_NOTES], [CUST_ORDER_NOTATION], [CUST_ORD_CREATEDATE]
	, [PartMaterialType], [MaterialType], [Alloy], [CO_AMOUNT], [UNIT_PRICE], [TOTAL_AMT_SHIPPED]
	, [COL_AMOUNT], [RESOURCE_ID_Raw], [OPERATION_TYPE], [StatusCurrOpType], [CURROPCHK], [RESOURCE_ID]
	, [AubPriText], [AubTcText], [HasAubPri], [HasAubTc], [TimeLimit], [LAST_CLOCK_OUT], [last_worked_seq]
	, [last_clocked], [LATE], [LATEP2], [LATEP1], [LATEP3], [LATEBP1], [WAIT_DAYS], [EMPLOYEE_ID]
	, [OPERATOR], [USER_1], [LEAD_SUPERVISOR], [SupervisorName], [StatusCurrOpMsg], [tdesc], [LABOR_TICKET_DESC]
	, [BUYER], [BUYERFILTER], [SALESREP_ID], [Account], [Plant_ID], [Tooling], [IsAP1], [IsAP2], [IsAP3]
	, [IsBP1], [IsUndetermined], [IsNORGRU], [IsTWMETA], [IsAMTRIB], [IsAUBMASK], [IsAUBPrime], [IsAUBSand]
	, [IsAUBsetup], [IsAUBTC], [IsAUBtakedown], [IsAUBPartMark], [IsAUBFinalInspection], [IsAUBInspection]
	, [IsAUBPenetrant], [IsAUBChemline], [AP1_ETRAC], [AP2_ETRAC], [AP3_ETRAC], [BP1_ETRAC], [AP1_PDEXP]
	, [AP2_PDEXP], [AP3_PDEXP], [BP1_PDEXP], [AP1_FLT], [AP2_FLT], [AP3_FLT], [BP1_FLT], [AP1_Buyer_Joe]
	, [AP1_Buyer_Rena], [AP1_Buyer_Tiffany], [AP1_Buyer_Sunni], [AP1_Buyer_Danielle], [AP1_Review]
	, [AP1_PlanningQueue], [AP1_SkyFlex], [ID], [DESIRED_RECV_DATE], [SnapshotDate], [DEPARTMENT], [SUB_DEPARTMENT]
	, [FACILITY], [REPT_DEPT], [PROCESS], [REPORT_GROUPING], [EMAIL_ADDR], [SETUP_INFO]
)

SELECT [BASE_ID], [LOT_ID], [SPLIT_ID], [SUB_ID], [TYPE], [PART_ID], [WONUM]
	, [DESIRED_QTY], [STATUS], [CUSTOMER_ORDER], [LINE_NO], [CUSTOMER_ID], [allocation_type]
	, [CurrentOperation], [StatusCurrOp], [PRINTED_DATE], [DESIRED_RLS_DATE], [CREATE_DATE]
	, [remaining_operations], [QTY_ON_HAND], [QTY_IN_DEMAND], [BUYER_USER_ID], [PLANNER_USER_ID]
	, [COMMODITY_CODE], [DESCRIPTION], [DESIRED_WANT_DATE], [EXP/FLT], [user_10], [col_user_1]
	, [RWPart], [CUSTOMER_PO_REF], [WORK_ORDER_NOTES], [CUST_ORDER_NOTATION], [CUST_ORD_CREATEDATE]
	, [PartMaterialType], [MaterialType], [Alloy], [CO_AMOUNT], [UNIT_PRICE], [TOTAL_AMT_SHIPPED]
	, [COL_AMOUNT], [RESOURCE_ID_Raw], [OPERATION_TYPE], [StatusCurrOpType], [CURROPCHK]
	, [RESOURCE_ID], [AubPriText], [AubTcText], [HasAubPri], [HasAubTc], [TimeLimit]
	, [LAST_CLOCK_OUT], [last_worked_seq], [last_clocked], [LATE], [LATEP2], [LATEP1], [LATEP3]
	, [LATEBP1], [WAIT_DAYS], [EMPLOYEE_ID], [OPERATOR], [USER_1], [LEAD_SUPERVISOR]
	, [SupervisorName], [StatusCurrOpMsg], [tdesc], [LABOR_TICKET_DESC], [BUYER], [BUYERFILTER]
	, [SALESREP_ID], [Account], [Plant_ID], [Tooling], [IsAP1], [IsAP2], [IsAP3], [IsBP1]
	, [IsUndetermined], [IsNORGRU], [IsTWMETA], [IsAMTRIB], [IsAUBMASK], [IsAUBPrime], [IsAUBSand]
	, [IsAUBsetup], [IsAUBTC], [IsAUBtakedown], [IsAUBPartMark], [IsAUBFinalInspection]
	, [IsAUBInspection], [IsAUBPenetrant], [IsAUBChemline], [AP1_ETRAC], [AP2_ETRAC]
	, [AP3_ETRAC], [BP1_ETRAC], [AP1_PDEXP], [AP2_PDEXP], [AP3_PDEXP], [BP1_PDEXP]
	, [AP1_FLT], [AP2_FLT], [AP3_FLT], [BP1_FLT], [AP1_Buyer_Joe], [AP1_Buyer_Rena]
	, [AP1_Buyer_Tiffany], [AP1_Buyer_Sunni], [AP1_Buyer_Danielle], [AP1_Review]
	, [AP1_PlanningQueue], [AP1_SkyFlex], [ID], [DESIRED_RECV_DATE], [SnapshotDate]
	, [DEPARTMENT], [SUB_DEPARTMENT], [FACILITY], [REPT_DEPT], [PROCESS], [REPORT_GROUPING]
	, [EMAIL_ADDR], [SETUP_INFO]
FROM (	SELECT [BASE_ID], [LOT_ID], [SPLIT_ID], [SUB_ID], [TYPE], [PART_ID], [WONUM]
			, [DESIRED_QTY], [STATUS], [CUSTOMER_ORDER], [LINE_NO], [CUSTOMER_ID]
			, [allocation_type], [CurrentOperation], [StatusCurrOp], [PRINTED_DATE]
			, [DESIRED_RLS_DATE], [CREATE_DATE], [remaining_operations], [QTY_ON_HAND]
			, [QTY_IN_DEMAND], [BUYER_USER_ID], [PLANNER_USER_ID], [COMMODITY_CODE]
			, [DESCRIPTION], [DESIRED_WANT_DATE], [EXP/FLT], [user_10], [col_user_1]
			, [RWPart], [CUSTOMER_PO_REF], [WORK_ORDER_NOTES], [CUST_ORDER_NOTATION]
			, [CUST_ORD_CREATEDATE], [PartMaterialType], [MaterialType], [Alloy], [CO_AMOUNT]
			, [UNIT_PRICE], [TOTAL_AMT_SHIPPED], [COL_AMOUNT], [RESOURCE_ID_Raw], [OPERATION_TYPE]
			, [StatusCurrOpType], [CURROPCHK], [RESOURCE_ID], [AubPriText], [AubTcText]
			, [HasAubPri], [HasAubTc], [TimeLimit], [LAST_CLOCK_OUT], [last_worked_seq]
			, [last_clocked], [LATE], [LATEP2], [LATEP1], [LATEP3], [LATEBP1], [WAIT_DAYS]
			, [EMPLOYEE_ID], [OPERATOR], [USER_1], [LEAD_SUPERVISOR], [SupervisorName]
			, [StatusCurrOpMsg], [tdesc], [LABOR_TICKET_DESC], [BUYER], [BUYERFILTER]
			, [SALESREP_ID], [Account], [Plant_ID], [Tooling], [IsAP1], [IsAP2], [IsAP3]
			, [IsBP1], [IsUndetermined], [IsNORGRU], [IsTWMETA], [IsAMTRIB], [IsAUBMASK]
			, [IsAUBPrime], [IsAUBSand], [IsAUBsetup], [IsAUBTC], [IsAUBtakedown], [IsAUBPartMark]
			, [IsAUBFinalInspection], [IsAUBInspection], [IsAUBPenetrant], [IsAUBChemline]
			, [AP1_ETRAC], [AP2_ETRAC], [AP3_ETRAC], [BP1_ETRAC], [AP1_PDEXP], [AP2_PDEXP]
			, [AP3_PDEXP], [BP1_PDEXP], [AP1_FLT], [AP2_FLT], [AP3_FLT], [BP1_FLT], [AP1_Buyer_Joe]
			, [AP1_Buyer_Rena], [AP1_Buyer_Tiffany], [AP1_Buyer_Sunni], [AP1_Buyer_Danielle]
			, [AP1_Review], [AP1_PlanningQueue], [AP1_SkyFlex], [ID], [DESIRED_RECV_DATE]
			, [SnapshotDate], [DEPARTMENT], [SUB_DEPARTMENT], [FACILITY], [REPT_DEPT], [PROCESS]
			, [REPORT_GROUPING], [EMAIL_ADDR], [SETUP_INFO]
...
DECLARE @TransactionStartDate DATETIME2(7) = SYSDATETIME()
	...
	, @UpdatedRows INT = 0
	, @DeletedRows INT = 0
	, @StoredProcedureName SYSNAME = (	SELECT @@SERVERNAME + '.' + DB_NAME() + '.' + s.name + '.' + o.name 
	...
	, @TargetTable SYSNAME = 'Wods_Output_new'
	, @StepId INT = 1
	, @Step VARCHAR(4000)

DECLARE @MergeResult TABLE (ActionTaken VARCHAR(20) NOT NULL);
										 
MERGE LiveSupplemental.dbo.WODS_Output_ETL Trgt
USING Staging.WODS_Output01_ETL SRC (NOLOCK)
ON Trgt.WONUM = SRC.WONUM

WHEN MATCHED AND trgt.BASE_ID <> src.BASE_ID OR trgt.LOT_ID <> src.LOT_ID
	OR trgt.SPLIT_ID <> src.SPLIT_ID OR trgt.SUB_ID <> src.SUB_ID
	OR trgt.TYPE <> src.TYPE OR ISNULL(trgt.PART_ID, '') <> ISNULL(src.PART_ID, '')
	...
	OR trgt.HasAubPri <> src.HasAubPri OR trgt.HasAubTc <> src.HasAubTc
	...
	OR trgt.LATE <> src.LATE OR trgt.LATEP2 <> src.LATEP2
	OR trgt.LATEP1 <> src.LATEP1 OR trgt.LATEP3 <> src.LATEP3
	OR trgt.LATEBP1 <> src.LATEBP1 OR trgt.WAIT_DAYS <> src.WAIT_DAYS
	...
	OR trgt.Account <> src.Account OR trgt.Plant_ID <> src.Plant_ID
	OR trgt.Tooling <> src.Tooling OR trgt.IsAP1 <> src.IsAP1
	OR trgt.IsAP2 <> src.IsAP2 OR trgt.IsAP3 <> src.IsAP3
	OR trgt.IsBP1 <> src.IsBP1 OR trgt.IsUndetermined <> src.IsUndetermined
	OR trgt.IsNORGRU <> src.IsNORGRU OR trgt.IsTWMETA <> src.IsTWMETA
	OR trgt.IsAMTRIB <> src.IsAMTRIB OR trgt.IsAUBMASK <> src.IsAUBMASK
	OR trgt.IsAUBPrime <> src.IsAUBPrime OR trgt.IsAUBSand <> src.IsAUBSand
	OR trgt.IsAUBsetup <> src.IsAUBsetup OR trgt.IsAUBTC <> src.IsAUBTC
	OR trgt.IsAUBtakedown <> src.IsAUBtakedown OR trgt.IsAUBPartMark <> src.IsAUBPartMark
	OR trgt.IsAUBFinalInspection <> src.IsAUBFinalInspection
	OR trgt.IsAUBInspection <> src.IsAUBInspection OR trgt.IsAUBPenetrant <> src.IsAUBPenetrant
	OR trgt.IsAUBChemline <> src.IsAUBChemline OR trgt.AP1_ETRAC <> src.AP1_ETRAC
	OR trgt.AP2_ETRAC <> src.AP2_ETRAC OR trgt.AP3_ETRAC <> src.AP3_ETRAC
	OR trgt.BP1_ETRAC <> src.BP1_ETRAC OR trgt.AP1_PDEXP <> src.AP1_PDEXP
	OR trgt.AP2_PDEXP <> src.AP2_PDEXP OR trgt.AP3_PDEXP <> src.AP3_PDEXP
	OR trgt.BP1_PDEXP <> src.BP1_PDEXP OR trgt.AP1_FLT <> src.AP1_FLT
	OR trgt.AP2_FLT <> src.AP2_FLT OR trgt.AP3_FLT <> src.AP3_FLT
	OR trgt.BP1_FLT <> src.BP1_FLT OR trgt.AP1_Buyer_Joe <> src.AP1_Buyer_Joe 
	OR trgt.AP1_Buyer_Rena <> src.AP1_Buyer_Rena OR trgt.AP1_Buyer_Tiffany <> src.AP1_Buyer_Tiffany 
	OR trgt.AP1_Buyer_Sunni <> src.AP1_Buyer_Sunni OR trgt.AP1_Buyer_Danielle <> src.AP1_Buyer_Danielle 
	OR trgt.AP1_Review <> src.AP1_Review OR trgt.AP1_PlanningQueue <> src.AP1_PlanningQueue 
	OR trgt.AP1_SkyFlex <> src.AP1_SkyFlex OR ISNULL(trgt.ID, '') <> ISNULL(src.ID, '') 
	OR ISNULL(trgt.DESIRED_RECV_DATE, '1/1/1990') <> ISNULL(src.DESIRED_RECV_DATE, '1/1/1990')
	OR trgt.SnapshotDate <> src.SnapshotDate OR ISNULL(trgt.DEPARTMENT, '') <> ISNULL(src.DEPARTMENT, '') 
...
WHEN NOT MATCHED BY TARGET
	THEN INSERT 
	 ( BASE_ID, LOT_ID, SPLIT_ID, SUB_ID, TYPE, PART_ID, WONUM, DESIRED_QTY, STATUS, CUSTOMER_ORDER
		, LINE_NO, CUSTOMER_ID, allocation_type, CurrentOperation, StatusCurrOp, PRINTED_DATE
		, DESIRED_RLS_DATE, CREATE_DATE, remaining_operations, QTY_ON_HAND, QTY_IN_DEMAND, BUYER_USER_ID
		, PLANNER_USER_ID, COMMODITY_CODE, DESCRIPTION, DESIRED_WANT_DATE, [EXP/FLT], user_10, col_user_1
		, RWPart, CUSTOMER_PO_REF, WORK_ORDER_NOTES, CUST_ORDER_NOTATION, CUST_ORD_CREATEDATE, PartMaterialType
		, MaterialType, Alloy, CO_AMOUNT, UNIT_PRICE, TOTAL_AMT_SHIPPED, COL_AMOUNT, RESOURCE_ID_Raw
		, OPERATION_TYPE, StatusCurrOpType, CURROPCHK, RESOURCE_ID, AubPriText, AubTcText, HasAubPri
		, HasAubTc, TimeLimit, LAST_CLOCK_OUT, last_worked_seq, last_clocked, LATE, LATEP2, LATEP1, LATEP3
		, LATEBP1, WAIT_DAYS, EMPLOYEE_ID, OPERATOR, USER_1, LEAD_SUPERVISOR, SupervisorName, StatusCurrOpMsg
		, tdesc, LABOR_TICKET_DESC, BUYER, BUYERFILTER, SALESREP_ID, Account, Plant_ID, Tooling, IsAP1, IsAP2
		, IsAP3, IsBP1, IsUndetermined, IsNORGRU, IsTWMETA, IsAMTRIB, IsAUBMASK, IsAUBPrime, IsAUBSand
		, IsAUBsetup, IsAUBTC, IsAUBtakedown, IsAUBPartMark, IsAUBFinalInspection, IsAUBInspection
		, IsAUBPenetrant, IsAUBChemline, AP1_ETRAC, AP2_ETRAC, AP3_ETRAC, BP1_ETRAC, AP1_PDEXP, AP2_PDEXP
		, AP3_PDEXP, BP1_PDEXP, AP1_FLT, AP2_FLT, AP3_FLT, BP1_FLT, AP1_Buyer_Joe, AP1_Buyer_Rena
		, AP1_Buyer_Tiffany, AP1_Buyer_Sunni, AP1_Buyer_Danielle, AP1_Review, AP1_PlanningQueue, AP1_SkyFlex
		, ID, DESIRED_RECV_DATE, SnapshotDate, DEPARTMENT, SUB_DEPARTMENT, FACILITY, REPT_DEPT, PROCESS
		, REPORT_GROUPING, EMAIL_ADDR  , [SETUP_INFO]
	)

	   
 
	VALUES ( BASE_ID, LOT_ID, SPLIT_ID, SUB_ID, TYPE, PART_ID, WONUM, DESIRED_QTY, STATUS, CUSTOMER_ORDER
			, LINE_NO, CUSTOMER_ID, allocation_type, CurrentOperation, StatusCurrOp, PRINTED_DATE
			, DESIRED_RLS_DATE, CREATE_DATE, remaining_operations, QTY_ON_HAND, QTY_IN_DEMAND, BUYER_USER_ID
			, PLANNER_USER_ID, COMMODITY_CODE, DESCRIPTION, DESIRED_WANT_DATE, [EXP/FLT], user_10, col_user_1
			, RWPart, CUSTOMER_PO_REF, WORK_ORDER_NOTES, CUST_ORDER_NOTATION, CUST_ORD_CREATEDATE, PartMaterialType
