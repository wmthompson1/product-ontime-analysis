---USE [LIVE] --  test on crunch0226
GO
-- \Work Order Reports\WorkOrderDailyStatus\SP_SK_ETL02_WS.0.sql

/****** 

1. tested \WorkOrderDailyStatus\SP_SK_ETL02_WS.sql on crunch
at time of test, was located in \Work Order Reports\WorkOrderDailyStatus\
rename to SP_SK_ETL02_WS.0
2. to push to prod, copy clean SP_SK_ETL02_WS to SP_SK_ETL02_WS.1
mark up and push to prod.
...
DECLARE @in_commodity_code_sand nvarchar(MAX) = N'standards,details'  ---- N'raw material';
DECLARE @in_commodity_code_rm nvarchar(MAX) = N'raw material';
...
SET NOCOUNT ON; 
	...
	IF OBJECT_ID('tempdb..#RAW_MATERIAL_WODsX') IS NOT NULL DROP TABLE #RAW_MATERIAL_WODsX
	IF OBJECT_ID('tempdb..#Prev_Op_WODs') IS NOT NULL DROP TABLE #Prev_Op_WODs
	-- #stands_dets_WODs'
	IF OBJECT_ID('tempdb..#stands_dets_WODs') IS NOT NULL DROP TABLE #stands_dets_WODs
	...
	and c.type = 'w'
	-- material only
	and a.SUBORD_WO_SUB_ID is null
...
-- arm/
	...
	INNER JOIN PART b WITH (NOLOCK) on a.PART_ID = b.ID  
		and b.COMMODITY_CODE IN (
    SELECT LTRIM(RTRIM(value))
    FROM STRING_SPLIT(@in_commodity_code_sand, ',')
    )
	--INNER JOIN OPERATION o on o.WORKORDER_BASE_ID=a.WORKORDER_BASE_ID and o.WORKORDER_LOT_ID=a.WORKORDER_LOT_ID and 
	--	o.WORKORDER_SPLIT_ID = a.WORKORDER_SPLIT_ID and o.SEQUENCE_NO = a.OPERATION_SEQ_NO
	--	and o.WORKORDER_SUB_ID = ISNULL(a.SUBORD_WO_SUB_ID, 0)
...
-- a/
	...
	SELECT a.BASE_ID, a.LOT_ID, a.SPLIT_ID, a.SUB_ID, a.TYPE, a.PART_ID, a.WONUM, a.DESIRED_QTY, a.STATUS, 
		...
		---- ~22
		(	SELECT TOP (1)
				 p.DESCRIPTION as 'OP_Description'
			FROM OPERATION o with (nolock)
		    ...
		    and o.RESOURCE_ID like '%PNT-PRIME'
				and o.SEQUENCE_NO = ( SELECT min(b.SEQUENCE_NO) FROM OPERATION b where b.WORKORDER_BASE_ID = a.BASE_ID 
		                             ...
		                             and b.RESOURCE_ID like '%PNT-PRIME' 
		                             and b.SEQUENCE_NO > a.StatusCurrOp )
		) as 'AubPriText',
		...
		--	 FROM OPERATION o WITH (NOLOCK)
		--	 JOIN OPERATION_TYPE p WITH (NOLOCK) ON o.RESOURCE_ID = p.RESOURCE_ID AND o.OPERATION_TYPE = p.ID
		--	 WHERE o.WORKORDER_BASE_ID = a.BASE_ID
		--	   AND o.STATUS NOT IN ('X','C')
		--	   AND o.RESOURCE_ID LIKE '%PNT-PRIME'
		--    and o.SEQUENCE_NO = ( SELECT min(b.SEQUENCE_NO) FROM OPERATION b where b.WORKORDER_BASE_ID = a.BASE_ID 
		...
		(	SELECT top (1)
			...
			INNER JOIN OPERATION_TYPE p with (nolock) ON o.RESOURCE_ID = p.RESOURCE_ID and o.OPERATION_TYPE = p.ID
			where o.WORKORDER_BASE_ID = a.BASE_ID 
		      and o.STATUS not in ('X', 'C') and o.RESOURCE_ID like '%PNT-TC'
				and o.SEQUENCE_NO = ( SELECT min(b.SEQUENCE_NO) FROM OPERATION b where b.WORKORDER_BASE_ID = a.BASE_ID 
		...
		/*  evaluate inventory into WO just to WO discard material issued */
				-- swap out bad code.
		,RM_READY  = iif(rX.available = 1, 'NO', NULL) 
	...
	) r ON r.WO = a.BASE_ID and a.LOT_ID = r.LOT and a.SPLIT_ID = r.SPLIT and a.SUB_ID = r.SUB
  ...
  -- c.
	...
	left join (
		...
		select * from #RAW_MATERIAL_WODsX

	) rX 
	ON a.BASE_ID = rX.WORKORDER_BASE_ID and a.LOT_ID = rX.WORKORDER_LOT_ID 
			and a.SPLIT_ID = rX.WORKORDER_SPLIT_ID and a.SUB_ID = rX.WORKORDER_SUB_ID
-- c./			
