...
- **UNIFIED abbreviated uid grammar — BOTH layers share ONE slot-5 form** (the
  ...
  - semantic example: `PAY_ELE_PAY_INV_<HASH8>` on key
    `PAYABLE:INVOICE_ID:semantic:Payables:elevates:PAY_ELE_PAY_INV_<HASH8>`.
  - Abbrev = first 3 alphanumeric chars uppercased. Collisions are EXPECTED
    (INVOICE_ID/INVENTORY → INV; cert_id/cert_type → CER).
  - **Two uid forms by layer (since M2/v14):** STRUCTURAL edges (has_column,
    references) use the COUNTED allocator — collisions resolved by a per-prefix
    uniqifier (`001`,`002`…), so the uid is ALLOCATED, not purely derived.
    SEMANTIC `elevates` uids are DERIVED instead: a readable prefix + sha1 hash of
    the natural key `(perspective,table,column,concept,field_component)` via
    `semantic_uid_stable()`. Deriving (not counting) is the M2 stability invariant —
    adding/removing a sibling concept on the same column+perspective never renumbers
    other elevates edges. The key still encodes the SOURCE column; concept on `_to`.
  ...
  - RESOLVED (M2/v14): the semantic-edge `_to` target is the CONCEPT NODE
    (`_from`=column → `_to`=concept_id); the ≤v13 self-loop is retired and the
    `concept` string is dropped from the edge (identity lives on `_to`).
...
- Current milestone `elevates_repointed_to_concepts` (SCHEMA_VERSION 14 /
  `graph_metadata.v14.json`): M2 of "concept as a node" — re-points the 17
  `elevates` edges to `column → concept node`, drops the `concept` string from the
  edge AND the `sql_graph_edges.concept` column (new `priority_weight` column
  carries the raw SME priority; binary `weight` is derived), and makes the elevates
  uid concept-aware/derived (above). Counts unchanged 279/279; live reloaded.
  (M1 `concept_nodes_introduced` / v13 added the identity-only concept nodes —
  name + description ONLY; type/domain/tags still deferred to M3.) App resolvers
  filter `node_type` in (`table`,`column`) so concept rows + re-pointed edges stay
  inert in the HF app (it reads the legacy named graph).
