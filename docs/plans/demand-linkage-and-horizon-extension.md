...
## Data-design rules (SME guidance)
- **Supply tracks demand closely**: when generating/extending synthetic supply (on-hand, scheduled receipts, planned orders), keep total supply very close to total demand per part — no large surpluses or artificial buffers.
- **Safety stock = 1** for planning parts. Use 1 (not 0, not a computed buffer) wherever a safety-stock value is seeded or consumed by netting.
